...
def pytest_addoption(parser):
    ...
    parser.addoption("--source-vrouter-id",action="store",default=None,help="Override source vrouter ID (int)")
    parser.addoption("--peer-vrouter-id",action="store",default=None,help="Override peer vrouter ID (int)")
    ...
    parser.addoption("--query",action="store",default=None,help="Override 'query' parameter value (e.g., wireguard_rx_bytes)")
...
@pytest.fixture(scope="session")
def valid_source_vrouter_id(request):
    cli_val = request.config.getoption("--source_vrouter_id")
    if cli_val:
        return cli_val
    env_file = request.getfixturevalue("get_env")
    load_dotenv(env_file, override=False)
    return os.getenv("VALID_SOURCE_VROUTER_ID")


@pytest.fixture(scope="session")
def valid_peer_vrouter_id(request):
    cli_val = request.config.getoption("--peer_vrouter_id")
    if cli_val:
        return cli_val
    env_file = request.getfixturevalue("get_env")
    load_dotenv(env_file, override=False)
    return os.getenv("VALID_PEER_VROUTER_ID")
...
# This will automatically override `query` in the default_params fixture dynamically
@pytest.fixture(autouse=True)
def override_query_param(request):
    # Get the --query CLI param
    cli_query = request.config.getoption("query")
    if not cli_query:
        # No override; do nothing
        yield
        return

    # Patch the default_params fixture's dictionary before use
    # Here we patch it via request.getfixturevalue and the param in session
    # That means your default_params fixture must be called after this runs (which it is)
    default_params = request.getfixturevalue("default_params")
    if default_params is not None:
        default_params["query"] = cli_query
    yield




# working for wireguard_connection_status starts here
...
def valid_time_to(request, get_env):
    ...
    return env_value
'''
# working for wireguard_connection_status ends here
